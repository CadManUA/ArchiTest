...
function OnSaveList(strFileName, tblPlanksData)
...
function OnSaveListElem(strFileName)
	local	i, v, tblElemGuid2Master, tblElemGuid2Owner, tblPlanksData, sId, plankinfo, elemMaster, elemOwner, tblPlank, nPlanks
...
function SummaryOptions()
	...
	s="\"1:Nothing\",\"2:3D surface material name\",\"3:3D surface material sample\",\"4:Quality (excel_quality from material list)\",\"5:Type (excel_type from material list)\""
	tblOptions[2]			={}
...
function GetSteelBeams(tblOthers)
	...
	for k,v in ipairs(tblSources) do
		...
		if tblElem.header.typeID==6 then
			...
			elseif string.find(s, "archiframesteelplate") then
				...
				if tblItem.matid==nil or tblItem.matid=="" then
					tblItem.matid=string.format("%s x %s x %s", ac_environment("ntos", tblItem.len, "length", "dim"), ac_environment("ntos", tblItem.width, "length", "dim"), ac_environment("ntos", tblItem.height, "length", "dim"))
				end
			end
		...
		if tblItem and tblItem.matid then
			sKey=string.format("%s %06.3f x %07.3f x %07.3f", tblItem.matid, tblItem.width, tblItem.height, tblItem.len)
...
function OnSaveListSummaryInt(strFileName)
	local	i, v, tblPlanksData, sId, plankinfo, tblPlank, nPlanks, tblHandledElems, tblElem, tblElemsWithPlanks, tblOthers, s, tbl
...
--		end
	...
	OnSaveList(strFileName, tblPlanksData)
end
...
function OnPreCollectCsvElem()
	...
	tblOptions[1].defvalue	=1
	tblOptions[1].valuelist="\"1:Include combined, skip sources (element split at building site)\",\"2:Include sources, skip combined (element split at factory)\""
...
function Planks2Xls( book, sheet )
	...
	while true do
		...
		if matLen~=0 and (bLastPlank or (matId~=prevMatId or math.abs(height-prevHeight)>af.EPS or math.abs(width-prevWidth)>af.EPS)) then
...
function Planks2XlsElemInt( tblMaster, book, sheet, rowNum, tblPlanks, addBreak )
	...
	-- Header
	projName=string.format( "%s, floor %d, element %s", GetAutoTextNoNil("<PROJECTNAME>"), tblMaster.floor, tblMaster.id )
	projNumber=GetAutoTextNoNil("<PROJECTNUMBER>")
		...
		if vElem.tblboards then
			...
			while vElem.tblboards[i] do
				...
				while vElem.tblboards[scan] do
					...
					if math.abs(vElem.tblboards[i].width-vElem.tblboards[scan].width)>0.001 or math.abs(vElem.tblboards[i].height-vElem.tblboards[scan].height)>0.001 then
						af.Log( string.format("ERROR: boards with same ID (%s) different size, update element and create listing again", vElem.tblboards[i].id) )
						break
					end
				...
				if book==nil or sheet==nil then
					-- To txt file
					io.write( string.format("%s\t%s\t%s\t%d\t%.3f\n", idStr, ac_environment("ntos", width, "length", "dim"), ac_environment("ntos", height, "length", "dim"), scan-i, (scan-i)*area) )
				else
...
--af.Log(string.format("wsHelp: i2=%s, s=%s", tostring(i2), tostring(s)))
		...
		io.write( string.format("PROJECT NAME:\t%s\n", projName) )
		io.write( string.format("PROJECT NUMBER:\t%s\n", projNumber) )
		io.write( string.format("Material ID\tWidth\tHeight\tPcs\tTotal len\tArea gross\tArea net\tTotal vol gr\tColour\tColour2\n", projNumber) )
	end
...
function Planks2XlsSummaryInt( book, sheet, rowNum, tblPlanks, othersNow )
	...
	while true do
		...
		if sheet==nil then
			...
			sLine=string.format("%s\t%s\t%s\t", matId, ac_environment("ntos", width, "length", "dim"), ac_environment("ntos", height, "length", "dim"))
...
--af.Log(string.format("type=%s val=%s", tostring(coltype), tostring(colval)))
				...
				if coltype and colval then
					if coltype==ESummaryInfoColourSample then
						local r, g, b, dummy
						dummy,dummy,r,g,b=string.find(colval, "([0123456789.]+),([0123456789.]+),([0123456789.]+)")
						if r and g and b then
							--- local color = book:color_pack(math.floor(255 * r + 0.5), math.floor(255 * g + 0.5), math.floor(255 * b + 0.5))
							local color = af.LibxlColor(math.floor(255 * r + 0.5), math.floor(255 * g + 0.5), math.floor(255 * b + 0.5))
							local fmt = book:add_format()
							if fmt ~= nil then
								if gXlsx then
									color=book:color_pack(math.floor(255 * r + 0.5), math.floor(255 * g + 0.5), math.floor(255 * b + 0.5))
...
function OnSaveListElemWork(strFileName)
	local	i, v, tblElemGuid2Master, tblElemGuid2Owner, tblPlanksData, sId, plankinfo, elemMaster, elemOwner, tblPlank, nPlanks, prevScript
...
function GetAccItem(elemid, tblAccItems, keywild)
	...
	if tblAccItems then			-- Cannot use for string keys: and #tblAccItems>0 
		...
		if data then
			-- Was it empty?
			af.Log(string.format("WARNING: Element %s: ArchiFrameAccessory-object is missing setting %s. Using default value 0", elemid, keywild))
		end
	end
...
--af.Log(string.format("vElem.elemtype=%s", vElem.elemtype))
		...
		elseif vElem.elemtype=="finish_ext" then 
			...
			planksMul=0
			if vElem.tblplanks then
				af.Log(string.format("WARNING: Element %s has exploded cladding - calculating cladding area hole from related ArchiFrameBoardPanel-objects only", tblMaster.id))
			end
			...
			if tblPolys1==nil or tblPolys1.area==0 then
				...
				af.Log(string.format("WARNING: Element %s does not have related ArchiFrameBoardPanel-objects - calculating cladding area from related ArchiFrameElement-object", tblMaster.id))
			end
...
--af.Log(string.format("area1=%f area2=%f", tblPolys1.area, tblPolys2.area))
			...
			for k,v in pairs(vElem.tblplanks) do
				...
				if t.kgpermeter then
					...
					if not gbVerWarned then
						gbVerWarned=true
						af.Log("WARNING: Program version does not give plank weight - estimating it here. Please update program.")
					end
	...
	if wb then
		...
		af.LibxlAreaToCell(wb, ws, rowNum, 1, claddingholesm2 + GetAccItem(tblMaster.id, tblAccItems, tblAccNames.row22_cladhole))
		rowNum=rowNum+1
